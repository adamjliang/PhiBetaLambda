          ...
          return SingleChildScrollView(
            ...
            child: 
              Column(
                children: [
                  SizedBox(height: 50),
                  (successfullyCreatedNewEvent)?Text('Successfully created new event!', style: TextStyle(fontSize: 18, color: Colors.green)):Container(),
              (successfullyCreatedNewEvent)?SizedBox(height: 50):Container(),
                ...
                child: 
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      errorText: _validateTitle ? 'Value Cannot Be Empty' : null,
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
                children: [ 
                  SizedBox(height: 25,
                  child: Center(child: Text('Start Date/Time',
                  ...
                  IconButton(
                    ...
                    onPressed: (){
                      setState(() {
                        (_startTimeOpen)?_startTimeOpen = false:_startTimeOpen = true;
              ...
              mainAxisAlignment: MainAxisAlignment.center,
                children: [ 
                  SizedBox(height: 25,
                  child: Center(child: Text('End Date/Time',
                  style: TextStyle(fontSize: 20)))),
                  (!_endTimeOpen)?
                  IconButton(
                    icon: (Icon(Icons.add)),
                    onPressed: (){
                      setState(() {
                        (_endTimeOpen)?_endTimeOpen = false:_endTimeOpen = true;
              ...
              (globals.level > 0)?Row(
                ...
                children: [
                  ...
                  DropdownButton<String>(
                    ...
                    items: <String>['Personal', 'PBL'].map((String value) {
                    ...
                    onChanged: (String newValue){
                      setState(() {
                        hintText = newValue;
                        (newValue == 'Personal')?calChoice = 0:calChoice = 1;
                      });
                    },
                  ),
                  Text(' Calendar',
              ...
              onTap: () {
               ...
               if (!_validateTitle && !(globals.level < 1 && calChoice == 1)){ //equiv of allValid
                  if (locationValue == ""){
                    locationValue = "N/A";
                  }
                  if (dresscodeValue == ""){
                  ...
                  setState(() {
                    ...
                    printedStartDate = _changeToReadableDate(_chosenStartDate);
                    printedEndDate = _changeToReadableDate(_chosenEndDate);
                    //printedDate += _chosenStartDate.toString().substring(startIndex)

                    Firestore.instance.collection("Calendar").document( '${(snapshot.data.documents[snapshot.data.documents.length-1]['entry'])}'  ).setData(
                      {'Title': '$titleValue', 
                      ...
                      'Entry': snapshot.data.documents[snapshot.data.documents.length-1]['entry'],
                      },
                    
                    );
                    int updateEntry = snapshot.data.documents[snapshot.data.documents.length-1]['entry'] + 1;
                    Firestore.instance.collection("Calendar").document('Entry').updateData({'entry': updateEntry});
                    titleValue = "";
                    locationValue = "";
                    dresscodeValue = "";
